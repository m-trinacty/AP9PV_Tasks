...
__global__ void processImag(unsigned char * out,unsigned char * in,size_t pitch, unsigned int width, unsigned int height ){
...
int main(){
    ...
    processImag<<<gridSize,blockSize>>>(d_r_n,d_r,pitch_r,width,height);
    processImag<<<gridSize,blockSize>>>(d_g_n,d_g,pitch_g,width,height);
    processImag<<<gridSize,blockSize>>>(d_b_n,d_b,pitch_b,width,height);
